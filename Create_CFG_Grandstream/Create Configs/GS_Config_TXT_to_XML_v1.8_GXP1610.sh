...
echo "  <config version=\"2\">" >> $1tmp1.xml
...
sed -e 's/^#\(.*$\)/<!--&-->/' -e 's/^-\(.*$\)/<!--&-->/' -e "s/^M//g" $1tmp2.xml >> $1tmp3.xml
...
while IFS=, read mac user password authid server accountname; do
...
#check the P value of Sip User ID in the txt file, and change the value in different conditions.
        ...
        sed -e "/^<mac>/s:.*:<mac>$mac</mac>:g" -e "/^<P35>/s:.*:<P35>$user</P35>:g" -e "/^<P270>/s:.*:<P270>$accountname</P270>:g"  -e "/^<P47>/s:.*:<P47>$server</P47>:g" -e "/^<P34>/s:.*:<P34>$password</P34>:g" -e "/^<P36>/s:.*:<P36>$authid</P36>:g" -e "/^<P/s:<P:    <P:" -e "s/^M//g" < $1tmp4.xml > "$1cfg$mac.xml"
        #openssl enc -e -aes-256-cbc -k Sl2005 -in cfg$mac.xml -out ENCcfg$mac.xml
        #mv ENCcfg$mac.xml cfg$mac.xml

        elif grep -q "<P4060>" $1tmp4.xml ; then

        sed -e "/^<mac>/s:.*:<mac>$mac</mac>:g" -e "/^<P4060>/s:.*:<P4060>$user</P4060>:g" -e "/^<P4120>/s:.*:<P4120>$password</P4120>:g" -e "/^<P4090>/s:.*:<P4090>$authid</P4090>:g" -e "/^<P/s:<P:    <P:" -e "s/^M//g" < $1tmp4.xml > "$1cfg$mac.xml"

        elif grep -q "<P3060>" $1tmp4.xml ; then

        sed -e "/^<mac>/s:.*:<mac>$mac</mac>:g" -e "/^<P3060>/s:.*:<P3060>$user</P3060>:g" -e "/^<P3120>/s:.*:<P3120>$password</P3120>:g" -e "/^<P3090>/s:.*:<P3090>$authid</P3090>:g" -e "/^<P/s:<P:    <P:" -e "s/^M//g" < $1tmp4.xml > "$1cfg$mac.xml"

        else
        sed -e "/^<mac>/s:.*:<mac>$mac</mac>:g" -e "/^<P/s:<P:    <P:" -e "s/^M//g" < $1tmp4.xml > "$1cfg$mac.xml"
        fi
else
